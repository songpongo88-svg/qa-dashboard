...
function CleanTeamOverviewPanel({
  teamGroups,
}: {
  teamGroups: TeamOverviewGroup[];
}) {
  const [selectedTeamName, setSelectedTeamName] =
    useState(teamGroups[0]?.teamName || "");
  ...
  const normalizedLead = normalizeUsername(
    selectedTeam?.teamLead || ""
  );

  const leadUser =
    selectedTeam?.users.find((user) =>
      [
        user.username,
        user.displayName,
        user.agentName || "",
    ...
    return [...selectedTeam.users]
      ...
      .sort((a, b) => {
        ...
        const roleDifference =
          (roleRank[getRowRole(a)] || 99) -
          (roleRank[getRowRole(b)] || 99);
  ...
  if (!teamGroups.length) {
    return (
      <div
        data-team-management-clean-v70="true"
      ...
      >
        ...
        <div className="mt-1 text-xs text-slate-400">
          เพิ่มชื่อทีมและสมาชิกจากเมนูจัดการทีม
        </div>
      </div>
    );
  }
  ...
  return (
    <div
      data-team-management-clean-v70="true"
      className="grid min-h-[620px] lg:grid-cols-[320px_minmax(0,1fr)]"
    >
      <aside className="border-b border-slate-200 bg-[#fcfcff] p-4 lg:border-b-0 lg:border-r">
        ...
        <div className="mt-4 text-[10px] font-semibold uppercase tracking-[0.18em] text-slate-400">
          ทีมทั้งหมด {visibleTeams.length}
        </div>
                ...
                {team.users
                  ...
                  .map((user) => (
                    <span
                      ...
                      className={`flex h-7 w-7 items-center justify-center rounded-[10px] border-2 border-white bg-gradient-to-br text-[9px] font-semibold text-white ${roleAvatarClass(
                        getRowRole(user)
                      )}`}
                    >
                      {userInitials(
      ...
      <main className="min-w-0 bg-gradient-to-b from-white to-slate-50 p-4 lg:p-5">
        {selectedTeam ? (
          <>
            <div className="rounded-[22px] border border-violet-100 bg-gradient-to-r from-violet-50 via-white to-fuchsia-50 p-4">
              <div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
                <div>
                  ...
                  <div className="mt-1 text-xs text-slate-500">
                    {selectedTeam.users.length} สมาชิก · Active{" "}
                    {selectedTeam.activeCount}
                    {selectedTeam.suspendedCount
                      ? ` · Suspended ${selectedTeam.suspendedCount}`
                      : ""}
                  </div>
                </div>

                <div className="flex items-center gap-3 rounded-2xl border border-white bg-white/90 px-3 py-2.5 shadow-sm">
                  <div
                    className={`flex h-11 w-11 items-center justify-center rounded-[15px] bg-gradient-to-br text-xs font-semibold text-white ${roleAvatarClass(
                      leadUser
                        ? getRowRole(leadUser)
                        : "Supervisor"
                    )}`}
              ...
              <div className="flex flex-col gap-3 border-b border-slate-200 px-4 py-3 sm:flex-row sm:items-center sm:justify-between">
                <div>
                  ...
                  <div className="mt-1 text-[11px] text-slate-400">
                    หัวหน้าทีมอยู่ลำดับแรก จากนั้นเรียงตาม Role
                  </div>
                </div>
                <input
                  value={memberSearch}
                  ...
                  title="ค้นหาชื่อ Username Role หรือสถานะ"
                  className="w-full rounded-xl border border-slate-200 px-3 py-2.5 text-xs outline-none focus:border-violet-500 focus:ring-4 focus:ring-violet-100 sm:max-w-[260px]"
                />
              </div>

              <div className="hidden grid-cols-[minmax(0,1.5fr)_150px_100px] gap-3 border-b border-slate-100 bg-slate-50 px-4 py-2.5 text-[10px] font-medium uppercase tracking-[0.12em] text-slate-400 md:grid">
                <div>ผู้ใช้งาน</div>
                <div>Role</div>
              ...
              <div>
                {orderedMembers.map((user) => {
                  ...
                  return (
                    <div
                      key={user.username}
                      className="grid gap-3 border-b border-slate-100 px-4 py-3 last:border-0 md:grid-cols-[minmax(0,1.5fr)_150px_100px] md:items-center"
                    >
                      <div className="flex min-w-0 items-center gap-3">
                        <div
                          className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-[14px] bg-gradient-to-br text-[11px] font-semibold text-white ${roleAvatarClass(
                            role
                          )}`}
                        >
                          {userInitials(
...
export default function UserRoleAdminMockup({
  ...
  const profileTeamRows = isSongponSuperAdmin
    ? rows
    : rows.filter((row) =>
        currentTeamName
          ? row.teamName === currentTeamName
          : normalizeUsername(row.username) ===
            normalizeUsername(currentUser.username)
      );
          ...
          <div className="mt-5">
            {userManagementView === "teams" ? (
              <div className="overflow-hidden rounded-[28px] border border-slate-200 bg-white shadow-[0_20px_60px_rgba(58,34,111,0.08)]">
                ...
                <CleanTeamOverviewPanel teamGroups={teamGroups} />
              </div>
            ) : userManagementView === "team-management" ? (
              <div
