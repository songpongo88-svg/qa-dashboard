...
function replaceRequired(source, before, after, label) {
  if (!source.includes(before)) {
    throw new Error(`Appeal PDF v42 anchor not found: ${label}`);
  }
...
function patchAppealCaseStatus() {
  ...
  source = replaceRequired(source, oldBlock, newBlock, "Appeal status source");
...
function patchPdfTopicTable() {
  ...
  source = replaceRequired(
    ...
    "Original to revised score display"
  );
  ...
  source = replaceRequired(
    ...
    "Non-appealed topic information"
  );
...
patchPdfTopicTable();
console.log("Patched Appeal PDF topic table: bilingual Description, เดิม → ใหม่ score, Case Detail status wording, merged Appeal Reason/Evaluation Comment, and non-appealed topics moved below the table.");
